...
public class BmsDailyController extends BaseController {
    ...
    public ApiResult<List<TbDaily>> list() {
        List<TbDaily> list = bmsDailyService.list();
        return ApiResult.success(list);
    }

    @PostMapping("/add")
    @ApiOperation(value = "添加日常记录")
    public ApiResult<TbDaily> save(@RequestBody TbDaily daily) {
    ...
    public ApiResult<TbDaily> find(@PathVariable("id") Integer id) {
        TbDaily byId = bmsDailyService.getById(id);
